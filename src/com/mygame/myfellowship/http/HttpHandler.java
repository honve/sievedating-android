...
import com.mygame.myfellowship.http.entityhandler.EntityCallBack;
import com.mygame.myfellowship.http.entityhandler.FileEntityHandler;
import com.mygame.myfellowship.http.entityhandler.StringEntityHandler;
...
public class HttpHandler<T> extends AsyncTask<Object, Object, Object> implements EntityCallBack {
    ...
    // 判断响应状态
    private CheckResponseInterface mCheckResponse;

    public void setCheckResponse(CheckResponseInterface checkResponse) {
    ...
    protected void onProgressUpdate(Object... values) {
        ...
        case UPDATE_FAILURE:
            if (callback != null) {
                Throwable error = (Throwable) values[1];
                int errorNo = (Integer) values[2];
                String errorMsg = (String) values[3];
                //
                if (error instanceof IOException) {
                    errorMsg = "网络异常，请检查您的网络。";
                } else if (error instanceof HttpResponseException) {
                    errorMsg = "网络异常，请稍后重试。";
                }

                callback.onFailure(error, errorNo, errorMsg);
            }
            break;
        case UPDATE_SUCCESS:
            if (callback != null) {
                callback.onSuccess((T) values[1]);
                callback.onSuccess((T) values[1],(HttpUriRequest)values[2],(HttpResponse)values[3]);
            }
            break;
        default:
            break;
